...
def _format_model_credential_error(exc: BaseException) -> str:
    return (
        "Personal assistant model authentication failed. "
        "Check the MODEL_ID mapping and API key in graph_src_v2/conf/settings.yaml.\n\n"
        f"Raw error: {exc}"
    )
...
def _format_model_connection_error(exc: BaseException) -> str:
    return (
        "Personal assistant model connection failed. "
        "Check model base_url and network reachability.\n\n"
        f"Raw error: {exc}"
    )


async def _run_personal_assistant(
    state: MessagesState,
    *,
    ...
    assistant = build_personal_assistant_agent(model)
...
_builder.add_node("run_personal_assistant", _run_personal_assistant)
_builder.add_edge(START, "run_personal_assistant")
_builder.add_edge("run_personal_assistant", END)

graph = _builder.compile(name="personal_assistant_demo")
