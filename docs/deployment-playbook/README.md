...
- 聊天类通用能力可走 passthrough；业务能力走平台语义 API（双通道）。
...
| 双通道混合（推荐增强） | Chat: Frontend -> passthrough -> LangGraph；Business: Frontend -> Platform API -> LangGraph | AI 只是平台一部分 | 不重复造轮子，扩展性最好 | 需要清晰路由边界 |
...
```text
                  ...
                  ▼                                           ▼
       ┌──────────────────────────┐                ┌──────────────────────────┐
       │ Next.js API Passthrough  │                │      Platform API         │
       │ (仅通用聊天/Agent能力)     │                │ (仅业务语义接口)           │
       └──────────────┬───────────┘                └──────────────┬───────────┘
                      │                                             │
                      └─────────────────────┬───────────────────────┘
                                            ▼
...
3. 通用聊天直接复用 `langgraph-nextjs-api-passthrough`，减少重复开发。
...
1. 先上线双通道骨架：聊天走 passthrough，业务走 Platform API。
