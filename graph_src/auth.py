...
def _extract_token(headers: Mapping[Any, Any]) -> str:
    ...
    if api_key:
        ...
        if mapped:
            return mapped
    raise Auth.exceptions.HTTPException(status_code=401, detail="Missing or invalid auth token")
...
@custom_auth.on
async def on_access(*, ctx: Auth.types.AuthContext, value: Auth.types.on.value) -> dict:
    user = _user_to_mapping(ctx.user)
    ensure_permission(user, ctx.resource, ctx.action)
    return {}


@custom_auth.on.threads.create
async def on_threads_create(
    *,
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    value["metadata"] = {**metadata, **owner_filter}
    return owner_filter


@custom_auth.on.threads
async def on_threads_scoped(*, ctx: Auth.types.AuthContext, value: Auth.types.on.threads.value) -> dict:
    user = _user_to_mapping(ctx.user)
    ensure_permission(user, "threads", ctx.action)
    return thread_owner_filter(user)


@custom_auth.on.assistants
async def on_assistants(*, ctx: Auth.types.AuthContext, value: Auth.types.on.assistants.value) -> dict:
    user = _user_to_mapping(ctx.user)
...
@custom_auth.on.store
async def on_store(*, ctx: Auth.types.AuthContext, value: Auth.types.on.store.value) -> dict:
    user = _user_to_mapping(ctx.user)
